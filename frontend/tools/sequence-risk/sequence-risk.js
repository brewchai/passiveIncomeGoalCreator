(() => {
    ...
    const el = {
        ...
        popupWithdrawalLabel: byId('popupWithdrawalLabel')
    };

    // ── Currency (USD / INR dual mode) ──
    let currency = 'USD';
    const INR_PER_USD = 94;
    function symChar() { return currency === 'INR' ? '₹' : '$'; }

    // ── Data Models ──
    const PRESETS = {
        great_depression: {
            name: 'Great Depression (1929)',
            desc: 'The worst economic downturn in modern history. Shows the impact of a massive, prolonged crash right at retirement. (30-year actuals, padded to 50)',
    ...
    const EMAIL_RE_DL = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

    const dlModal = byId('downloadModal');
    const openDlBtn = byId('openDownloadModal');
    const closeDlBtn = byId('closeDownloadModal');
    const dlForm = byId('downloadEmailForm');
    const dlEmailInput = byId('downloadEmail');
    const dlSubmitBtn = byId('downloadSubmitBtn');
    const dlError = byId('downloadEmailError');

    function buildCSV() {
        if (!state.lastResults) return '';
        const { inputs, currentResult } = state.lastResults;
        const { isGuardrails } = inputs;
        const rows = currentResult.rows;

        const cur = symChar();
        const headers = isGuardrails
            ? ['Year', `Start Portfolio (${cur})`, 'Withdrawal Rate (%)', `Withdrawal (${cur})`, 'Return (%)', `End Portfolio (${cur})`, 'End WR (%)', 'Guardrail Trigger', `Next Year Withdrawal (${cur})`]
            : ['Year', `Start Portfolio (${cur})`, 'Withdrawal Rate (%)', `Withdrawal (${cur})`, 'Return (%)', `End Portfolio (${cur})`];
        ...
        for (const r of rows) {
            const base = [
                r.year,
                r.startPort.toFixed(0),
                (r.startWR * 100).toFixed(2),
            ...
            if (isGuardrails) {
                base.push(
                    (r.endWR * 100).toFixed(2),
                    `"${r.trigger}"`,
                    r.nextW > 0 ? r.nextW.toFixed(0) : '-'
                );
            }
        ...
        // Metadata footer
        lines.push('');
        lines.push(`"Generated by But First Fire — butfirstfire.com"`);
        lines.push(`"Scenario: ${inputs.years}-year retirement | Starting portfolio: ${fmtMoney(inputs.startVal)} | Initial withdrawal: ${fmtMoney(inputs.initialWithdrawal)} | Inflation: ${inputs.inflPct}% | Avg return: ${inputs.avgPct}%"`);
        lines.push(`"Strategy: ${isGuardrails ? 'Guardrails (Guyton-Klinger)' : 'Constant Purchasing Power'}"`);
    ...
    function triggerCSVDownload() {
        ...
        a.href = url;
        a.download = 'butfirstfire-guardrails-plan.csv';
    ...
    if (openDlBtn) {
        openDlBtn.addEventListener('click', () => {
            if (!state.lastResults) return;
            if (dlEmailInput) dlEmailInput.value = '';
            if (dlError) dlError.style.display = 'none';
            if (dlModal) dlModal.style.display = 'flex';
            setTimeout(() => { if (dlEmailInput) dlEmailInput.focus(); }, 100);
        });
    }

    if (closeDlBtn) closeDlBtn.addEventListener('click', () => { if (dlModal) dlModal.style.display = 'none'; });
    if (dlModal) dlModal.addEventListener('click', (e) => { if (e.target === dlModal) dlModal.style.display = 'none'; });

    if (dlForm) {
        dlForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const email = dlEmailInput ? dlEmailInput.value.trim() : '';
            if (!EMAIL_RE_DL.test(email)) {
            ...
            if (dlSubmitBtn) { dlSubmitBtn.disabled = true; dlSubmitBtn.textContent = 'Downloading…'; }

            // Submit email to newsletter (fire-and-forget, same as blog.html)
            try {
                const fd = new URLSearchParams();
                fd.append('email', email);
                fd.append('source', 'sequence-risk-calculator-download');
                await fetch(NEWSLETTER_ENDPOINT, { method: 'POST', mode: 'no-cors', body: fd });
            } catch (_) { /* never block download on webhook failure */ }
            ...
            if (dlSubmitBtn) { dlSubmitBtn.disabled = false; dlSubmitBtn.textContent = 'Download CSV →'; }
        });
    }

    // ── Currency toggle wiring ──
    function updateCurrencySymbols() {
        document.querySelectorAll('.currency-symbol').forEach(s => { s.textContent = symChar(); });
        if (el.popupWithdrawalLabel) el.popupWithdrawalLabel.textContent = `Withdrawal (${symChar()})`;
    }

    function setCurrency(newCur) {
        if (newCur === currency) return;
        const factor = newCur === 'INR' ? INR_PER_USD : (1 / INR_PER_USD);
        ['startingPortfolio', 'annualWithdrawal', 'guardrailsFloor'].forEach(id => {
            const e = byId(id);
            if (e && e.value !== '') e.value = Math.round(safeNum(e.value, 0) * factor);
        });
        currency = newCur;
        ...
        if (activePreset !== 'custom') showPreview(activePreset);
    }

    if (el.curBtns) el.curBtns.forEach(b => b.addEventListener('click', () => setCurrency(b.dataset.cur)));
