(() => {
    ...
    function buildCSV() {
        ...
        const headers = isGuardrails
            ? ['Year', 'Start Portfolio ($)', 'Withdrawal Rate (%)', 'Withdrawal ($)', 'Return (%)', 'End Portfolio ($)', 'End WR (%)', 'Guardrail Trigger', 'Next Year Withdrawal ($)']
            : ['Year', 'Start Portfolio ($)', 'Withdrawal Rate (%)', 'Withdrawal ($)', 'Return (%)', 'End Portfolio ($)'];
        ...
        lines.push(`"Scenario: ${inputs.years}-year retirement | Starting portfolio: $${inputs.startVal.toLocaleString()} | Initial withdrawal: $${inputs.initialWithdrawal.toLocaleString()} | Inflation: ${inputs.inflPct}% | Avg return: ${inputs.avgPct}%"`);
